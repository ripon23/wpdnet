...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_post_default'); ?>>
	...
	<?php
		$date ? charity_ngo_get_post_date('page', 'default') : '';
		
		charity_ngo_post_heading(get_the_ID(), 'h2');
		...
		if (!post_password_required()) {
			if ($cmsmasters_post_image_link != '') {
				charity_ngo_thumb(get_the_ID(), 'cmsmasters-masonry-thumb', false, 'img_' . get_the_ID(), false, false, false, true, $cmsmasters_post_image_link);
			} elseif (has_post_thumbnail()) {
				charity_ngo_thumb(get_the_ID(), 'cmsmasters-masonry-thumb', false, 'img_' . get_the_ID(), false, false, false, true, false);
			}
		}
