...
$excerpt = (in_array('excerpt', $cmsmasters_project_metadata) && charity_ngo_project_check_exc_cont()) ? true : false;
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_project_grid'); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		...
		charity_ngo_thumb_rollover(get_the_ID(), 'cmsmasters-project-grid-thumb', false, $rollover, false, false, false, false, false, false, true, $cmsmasters_project_link_redirect, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
		...
		$excerpt ? charity_ngo_project_exc_cont() : '';
		...
		if (!$title) {
			echo '<div class="dn">';
				charity_ngo_project_heading(get_the_ID(), 'h6');
			echo '</div>';
		}
