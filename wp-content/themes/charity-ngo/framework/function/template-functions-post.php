...
function charity_ngo_post_heading($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function charity_ngo_post_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
	$out = '';
...
function charity_ngo_get_post_date($template_type = 'page', $layout_type = 'default', $show = true) {
...
function charity_ngo_get_post_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_post_author">' . 
			'<span class="cmsmasters_post_author_suf">' . esc_html__('by', 'charity-ngo') . ' </span>' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'charity-ngo') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author">' . 
		...
		if ($cmsmasters_option['charity-ngo' . '_blog_post_author']) {
			$out .= '<span class="cmsmasters_post_author">' . 
...
function charity_ngo_get_post_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			$out .= '<span class="cmsmasters_post_category">' . 
				'<span class="cmsmasters_post_cat_suf">' . esc_html__('in', 'charity-ngo') . ' </span>' . 
			...
			if ($cmsmasters_option['charity-ngo' . '_blog_post_cat']) {
				$out .= '<span class="cmsmasters_post_category">' . 
...
function charity_ngo_get_post_tags($show = true) {
	if (get_the_tags()) {
		$cmsmasters_option = charity_ngo_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['charity-ngo' . '_blog_post_tag']) {
			$out .= '<span class="cmsmasters_post_tags">' . 
				get_the_tag_list(esc_html__('tags', 'charity-ngo') . ' ', ', ', '') . 
			'</span>';
		}
...
function charity_ngo_post_exc_cont($show = true) {
...
function charity_ngo_get_post_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['charity-ngo' . '_blog_post_like']) {
			$out = cmsmasters_like('cmsmasters_post_likes');
		}
	}
...
function charity_ngo_get_post_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = charity_ngo_get_comments('cmsmasters_post_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = charity_ngo_get_global_options();
			
			if ($cmsmasters_option['charity-ngo' . '_blog_post_comment']) {
				$out = charity_ngo_get_comments('cmsmasters_post_comments');
			}
		}
	}
...
function charity_ngo_post_more($cmsmasters_id, $show = true) {
	$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
	
	
	if ($cmsmasters_post_read_more == '') {
		$cmsmasters_post_read_more = esc_html__('Read More', 'charity-ngo');
	}
...
function charity_ngo_post_format_icon_placeholder($cmsmasters_id, $format = 'std', $show = true) {
