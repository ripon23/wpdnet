...
function charity_ngo_slider_post_heading($cmsmasters_id, $type = 'post', $tag = 'h1', $link_redirect = false, $link_url = false, $show = true) { 
	$out = '';
...
function charity_ngo_slider_post_exc_cont($type = 'post', $show = true) {
	if ($type == 'post') {
...
function charity_ngo_slider_post_check_exc_cont($type = 'post') {
	$exc = charity_ngo_slider_post_exc_cont($type, false);
...
function charity_ngo_get_slider_post_date($type = 'post', $show = true) {
	if ($type == 'post') {
...
function charity_ngo_get_slider_post_author($type = 'post', $show = true) {
	if ($type == 'post') {
		$out = '<span class="cmsmasters_slider_post_author">' . 
			esc_html__('By', 'charity-ngo') . ' ' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Posts by', 'charity-ngo') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author"><span class="fn" rel="author">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($type == 'project') {
...
function charity_ngo_get_slider_post_category($cmsmasters_id, $taxonomy, $type = 'post', $show = true) {
	$out = '';
...
function charity_ngo_slider_post_like($type = 'post', $show = true) {
	$out = '';
...
function charity_ngo_get_slider_post_comments($type = 'post', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($type == 'post') {
			$out = charity_ngo_get_comments('cmsmasters_slider_post_comments');
		} elseif ($type == 'project') {
			$out = charity_ngo_get_comments('cmsmasters_slider_project_comments');
		}
	}
...
function charity_ngo_slider_post_more($cmsmasters_id, $post_format = 'post', $show = true) {
	if ($post_format == 'post') {
		$cmsmasters_post_read_more = get_post_meta($cmsmasters_id, 'cmsmasters_post_read_more', true);
		
		
		if ($cmsmasters_post_read_more == '') {
			$cmsmasters_post_read_more = esc_attr__('Learn More...', 'charity-ngo');
		}
		...
		if ($cmsmasters_project_read_more == '') {
			$cmsmasters_project_read_more = esc_attr__('Learn More...', 'charity-ngo');
		}
