...
function charity_ngo_get_page_classes($cmsmasters_option, $classes = false) {
	...
	echo 'cmsmasters_' . $cmsmasters_option['charity-ngo' . '_theme_layout'] . ' ';

	if ($cmsmasters_option['charity-ngo' . '_fixed_header']) {
		echo 'fixed_header ';
	}

	if ($cmsmasters_option['charity-ngo' . '_header_top_line']) {
		echo 'enable_header_top ';
	}

	if ($cmsmasters_option['charity-ngo' . '_header_styles'] != 'default' && $cmsmasters_option['charity-ngo' . '_header_styles'] != 'fullwidth') {
		echo 'enable_header_bottom ';
	}

	if ($cmsmasters_option['charity-ngo' . '_header_styles'] == 'r_nav') {
		echo 'enable_header_right ';
	}

	if ($cmsmasters_option['charity-ngo' . '_header_styles'] == 'c_nav') {
	...
	if ($cmsmasters_header_overlaps == '') {
		$cmsmasters_header_overlaps = $cmsmasters_option['charity-ngo' . '_header_overlaps'];
	}
...
function charity_ngo_header_top($cmsmasters_option) {
	if ($cmsmasters_option['charity-ngo' . '_header_top_line']) { 
		echo '<div class="header_top" data-height="' .  esc_attr($cmsmasters_option['charity-ngo' . '_header_top_height']) . '">' . 
			'<div class="header_top_outer">' . 
				'<div class="header_top_inner">';
				
					if (
						$cmsmasters_option['charity-ngo' . '_header_top_line_add_cont'] !== 'none'
					) {
						echo '<div class="header_top_right">';
						
						
						if (
							$cmsmasters_option['charity-ngo' . '_header_top_line_add_cont'] == 'social' && 
							isset($cmsmasters_option['charity-ngo' . '_social_icons'])
						) {
							charity_ngo_social_icons();
						} elseif (
							$cmsmasters_option['charity-ngo' . '_header_top_line_add_cont'] == 'nav' && 
					...
					if ($cmsmasters_option['charity-ngo' . '_header_top_line_short_info'] !== '') {
						echo '<div class="header_top_left">' . 
							'<div class="meta_wrap">' . 
								stripslashes($cmsmasters_option['charity-ngo' . '_header_top_line_short_info']) . 
							'</div>' . 
						'</div>';
					} 
...
function charity_ngo_header_mid($cmsmasters_option) {
	echo '<div class="header_mid" data-height="' . esc_attr($cmsmasters_option['charity-ngo' . '_header_mid_height']) . '">' . 
		'<div class="header_mid_outer">' . 
			'<div class="header_mid_inner'; 
			if (!(
				CMSMASTERS_DONATIONS && 
				class_exists('Cmsmasters_Donations') && 
				isset($cmsmasters_option['charity-ngo' . '_header_donations_but']) && 
				$cmsmasters_option['charity-ngo' . '_header_donations_but'] && 
				isset($cmsmasters_option['charity-ngo' . '_header_donations_but_text']) && 
				$cmsmasters_option['charity-ngo' . '_header_donations_but_text'] != '' && 
				$cmsmasters_option['charity-ngo' . '_header_styles'] != 'c_nav'
			)) {
				echo ' cmsmasters_no_donation';
			}	
			echo '">' . 
				'<div class="logo_wrap">';
					...
					'<div class="header_donation_but_wrap_inner">' . 
						'<div class="header_donation_but">' . 
							'<a href="' . esc_url(get_permalink($cmsmasters_donations_form_page)) . '" class="cmsmasters_button">' . 
								'<span>' . esc_html($cmsmasters_option['charity-ngo' . '_header_donations_but_text']) . '</span>' . 
							'</a>' . 
						'</div>' . 
					'</div>' . 
				'</div>';
			...
			if (
				...
				if (
					$cmsmasters_option['charity-ngo' . '_header_add_cont'] == 'cust_html' && 
					$cmsmasters_option['charity-ngo' . '_header_add_cont_cust_html'] !== ''
				) {
				echo '<div class="slogan_wrap">' . 
					'<div class="slogan_wrap_inner">' . 
						'<div class="slogan_wrap_text">' . 
							stripslashes($cmsmasters_option['charity-ngo' . '_header_add_cont_cust_html']) . 
						'</div>' . 
					'</div>' . 
				'</div>';
				} elseif (
					$cmsmasters_option['charity-ngo' . '_header_add_cont'] == 'social' && 
					isset($cmsmasters_option['charity-ngo' . '_social_icons'])
				) {
					charity_ngo_social_icons();
				}
			...
			if (
				...
				echo '<div class="cmsmasters_dynamic_cart_wrap">';
					charity_ngo_woocommerce_cart_dropdown(); 
				echo '</div>';
			}
			
			
			if (
				$cmsmasters_option['charity-ngo' . '_header_search'] && 
				$cmsmasters_option['charity-ngo' . '_header_styles'] != 'c_nav'
			) {
			...
			if (
				$cmsmasters_option['charity-ngo' . '_header_styles'] == 'default' || 
				$cmsmasters_option['charity-ngo' . '_header_styles'] == 'fullwidth'
			) { 
				...
				'<div class="mid_nav_wrap">' . 
					'<nav >';
						...
						$nav_args = array( 
							...
							'link_before' => 		'<span class="nav_item_wrap">', 
							'link_after' => 		'</span>', 
							'fallback_cb' => 		'charity_ngo_fallback_menu' 
						);
...
function charity_ngo_header_bot($cmsmasters_option) {
	if (
		$cmsmasters_option['charity-ngo' . '_header_styles'] != 'default' &&
		$cmsmasters_option['charity-ngo' . '_header_styles'] != 'fullwidth'
	) {
		echo '<div class="header_bot" data-height="' . esc_attr($cmsmasters_option['charity-ngo' . '_header_bot_height']) . '">' . 
...
/* Get Logo Function */
function charity_ngo_logo() {
	$cmsmasters_option = charity_ngo_get_global_options();
	
	
	if ($cmsmasters_option['charity-ngo' . '_logo_type'] == 'text') {
		if ($cmsmasters_option['charity-ngo' . '_logo_title'] != '') {
			$blog_title = $cmsmasters_option['charity-ngo' . '_logo_title'];
		} else {
			$blog_title = (get_bloginfo('name')) ? get_bloginfo('name') : 'Charity NGO';
		}
		
		
		if ($cmsmasters_option['charity-ngo' . '_logo_subtitle'] != '') {
			$blog_descr = $cmsmasters_option['charity-ngo' . '_logo_subtitle'];
		} else {
			$blog_descr = (get_bloginfo('description')) ? get_bloginfo('description') : esc_html__('Default Logo Subtitle', 'charity-ngo');
		}
		...
		echo '<a href="' . esc_url(home_url('/')) . '" title="' . esc_attr($blog_title) . '" class="logo">' . "\n\t" . 
			'<span class="logo_text_wrap">' . 
				...
				($cmsmasters_option['charity-ngo' . '_logo_subtitle'] ? '<span class="title_text">' . esc_html($blog_descr) . '</span>' : '') . 
			'</span>' . 
		'</a>';
	} else {
		...
		if ($cmsmasters_option['charity-ngo' . '_logo_url'] == '') {
			...
			$logo_img = explode('|', $cmsmasters_option['charity-ngo' . '_logo_url']);
			...
			if ($cmsmasters_option['charity-ngo' . '_logo_url_retina'] != '') {
				$logo_img_retina = explode('|', $cmsmasters_option['charity-ngo' . '_logo_url_retina']);
...
/* Logo Styles */
function charity_ngo_theme_logo_styles() {
	$cmsmasters_option = charity_ngo_get_global_options();
	...
	if ($cmsmasters_option['charity-ngo' . '_logo_type'] == 'text') {
		if ($cmsmasters_option['charity-ngo' . '_logo_custom_color']) {
			$out .= "
		...
		$header_mid_height = (($cmsmasters_option['charity-ngo' . '_header_mid_height'] !== '') ? $cmsmasters_option['charity-ngo' . '_header_mid_height'] : '108');
...
add_action('wp_enqueue_scripts', 'charity_ngo_theme_logo_styles');



/* Get Footer Logo Function */
function charity_ngo_footer_logo($cmsmasters_option) {
	if (
		$cmsmasters_option['charity-ngo' . '_footer_type'] == 'default' && 
		$cmsmasters_option['charity-ngo' . '_footer_logo']
	) {
		list($logo_footer_width, $logo_footer_height) = getimagesize(get_template_directory() . '/img/logo_footer.png');
		
		
		if ($cmsmasters_option['charity-ngo' . '_footer_logo_url'] == '') {
			...
			$footer_logo_img = explode('|', $cmsmasters_option['charity-ngo' . '_footer_logo_url']);
			...
			if ($cmsmasters_option['charity-ngo' . '_footer_logo_url_retina'] != '') {
				$footer_logo_img_retina = explode('|', $cmsmasters_option['charity-ngo' . '_footer_logo_url_retina']);
...
function charity_ngo_get_footer_custom_html($cmsmasters_option) {
	if (
		(
			$cmsmasters_option['charity-ngo' . '_footer_type'] == 'default' && 
			$cmsmasters_option['charity-ngo' . '_footer_html'] !== ''
		) || (
			$cmsmasters_option['charity-ngo' . '_footer_type'] == 'small' && 
			$cmsmasters_option['charity-ngo' . '_footer_additional_content'] == 'text' && 
			$cmsmasters_option['charity-ngo' . '_footer_html'] !== ''
		)
	) {
		echo '<div class="footer_custom_html_wrap">' . 
			'<div class="footer_custom_html">' . 
				do_shortcode(wp_kses(stripslashes($cmsmasters_option['charity-ngo' . '_footer_html']), 'post')) .  
			'</div>' . 
		'</div>';
	}
}
...
function charity_ngo_get_footer_nav($cmsmasters_option) {
	if (
		has_nav_menu('footer') && 
		(
			(
				$cmsmasters_option['charity-ngo' . '_footer_type'] == 'default' && 
				$cmsmasters_option['charity-ngo' . '_footer_nav']
			) || (
				$cmsmasters_option['charity-ngo' . '_footer_type'] == 'small' && 
				$cmsmasters_option['charity-ngo' . '_footer_additional_content'] == 'nav'
			)
		)
	) {
...
function charity_ngo_get_footer_social_icons($cmsmasters_option) {
	if (
		isset($cmsmasters_option['charity-ngo' . '_social_icons']) && 
		(
			(
				$cmsmasters_option['charity-ngo' . '_footer_type'] == 'default' && 
				$cmsmasters_option['charity-ngo' . '_footer_social']
			) || (
				$cmsmasters_option['charity-ngo' . '_footer_type'] == 'small' && 
				$cmsmasters_option['charity-ngo' . '_footer_additional_content'] == 'social'
			)
		)
	) {
		charity_ngo_social_icons();
...
function charity_ngo_page_heading() {
	$cmsmasters_option = charity_ngo_get_global_options();
	...
	if (
		...
		$cmsmasters_header_overlaps = $cmsmasters_option['charity-ngo' . '_header_overlaps'] ? 'true' : 'false';
		
		$cmsmasters_heading_alignment = $cmsmasters_option['charity-ngo' . '_heading_alignment'];
		$cmsmasters_heading_scheme = $cmsmasters_option['charity-ngo' . '_heading_scheme'];
		
		$cmsmasters_breadcrumbs = $cmsmasters_option['charity-ngo' . '_breadcrumbs'] ? 'true' : 'false';
	}
			...
			if ($cmsmasters_heading == 'custom') {
				...
				if (is_search()) {
					...
					if (!empty($wp_query->found_posts)) {
						echo sprintf(esc_html(_n('1 search result for: %2$s', '%1$d search results for: %2$s', $wp_query->found_posts, 'charity-ngo')), $wp_query->found_posts, get_search_query());
					} else {
						echo sprintf(esc_html__('0 search results for: %s', 'charity-ngo'), get_search_query());
					}
				} elseif (is_archive()) {
					if (is_author()) {
						if (get_the_author_meta('first_name') != '' || get_the_author_meta('last_name') != '') {
							echo sprintf(esc_html__('Author: %1$s (%2$s %3$s)', 'charity-ngo'), '<span class="vcard">' . get_the_author() . '</span>', get_the_author_meta('first_name'), get_the_author_meta('last_name'));
						} else {
							echo sprintf(esc_html__('Author: %s', 'charity-ngo'), '<span class="vcard">' . get_the_author() . '</span>');
						}
					} else if (
...
/* Page Heading Styles */
function charity_ngo_theme_page_heading_styles() {
	$cmsmasters_option = charity_ngo_get_global_options();
	...
	if (
		...
		$cmsmasters_heading_height = $cmsmasters_option['charity-ngo' . '_heading_height'];
		$cmsmasters_heading_bg_color = $cmsmasters_option['charity-ngo' . '_heading_bg_color'];
		$cmsmasters_heading_bg_img_enable = $cmsmasters_option['charity-ngo' . '_heading_bg_image_enable'] ? 'true' : 'false';
		$cmsmasters_heading_bg_img = $cmsmasters_option['charity-ngo' . '_heading_bg_image'];
		$cmsmasters_heading_bg_rep = $cmsmasters_option['charity-ngo' . '_heading_bg_repeat'];
		$cmsmasters_heading_bg_att = $cmsmasters_option['charity-ngo' . '_heading_bg_attachment'];
		$cmsmasters_heading_bg_size = $cmsmasters_option['charity-ngo' . '_heading_bg_size'];
	}
...
add_action('wp_enqueue_scripts', 'charity_ngo_theme_page_heading_styles');


/* Get Social Icons Function */
function charity_ngo_social_icons() {
	$cmsmasters_option = charity_ngo_get_global_options();
...
<div class=\"social_wrap\">
	...
	foreach ($cmsmasters_option['charity-ngo' . '_social_icons'] as $cmsmasters_social_icons) {
...
function charity_ngo_theme_social_icons_styles() {
	$cmsmasters_option = charity_ngo_get_global_options();
...
add_action('wp_enqueue_scripts', 'charity_ngo_theme_social_icons_styles');


/* Get Posts Thumbnail Function */
function charity_ngo_thumb($cmsmasters_id, $type = 'post-thumbnail', $link = true, $group = false, $preload = true, $highImg = false, $fullwidth = true, $show = true, $attachment = false, $unique = false, $link_icon = false, $placeholder_icon = 'cmsmasters_theme_icon_image') {
	$args = array( 
...
function charity_ngo_thumb_small($cmsmasters_id, $type = 'post', $w = 100, $h = 100, $show = true) {
...
/* Get Heading Function */
function charity_ngo_get_comments($class = false, $show = false) { 
	$out = '';
	
	
	if (comments_open()) {
		$out .= '<span class="cmsmasters_comments' . ($class ? ' ' . $class : '') . '">' . 
			'<a class="cmsmasters_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'charity-ngo') . ' ' . esc_attr(get_the_title()) . '">' . 
...
function charity_ngo_prev_next_posts() {
	...
	if ($published_posts > 1) {
		...
		previous_post_link('<span class="cmsmasters_prev_post"><span class="post_nav_sub">' . esc_html__('Previous', 'charity-ngo') . '<span class="post_nav_type"> ' . $cmsmasters_post_type . ' </span>' . esc_html__('Link', 'charity-ngo') . '</span>%link<span class="cmsmasters_prev_arrow"><span></span></span></span>');
		
		next_post_link('<span class="cmsmasters_next_post"><span class="post_nav_sub">' . esc_html__('Next', 'charity-ngo') . '<span class="post_nav_type"> ' . $cmsmasters_post_type . ' </span>' . esc_html__('Link', 'charity-ngo') . '</span>%link<span class="cmsmasters_next_arrow"><span></span></span></span>');
...
function charity_ngo_sharing_box($title_box = false, $tag = 'h3') {
	...
	echo "<aside class=\"share_posts\">
		...
		<div class=\"share_posts_inner\">
			<a href=\"https://www.facebook.com/sharer/sharer.php?display=popup&u={$page_link}\">" . esc_html__('Facebook', 'charity-ngo') . "</a>
			<a href=\"https://plus.google.com/share?url={$page_link}\">" . esc_html__('Google+', 'charity-ngo') . "</a>
			<a href=\"https://twitter.com/intent/tweet?text=" . urlencode(html_entity_decode(sprintf(esc_attr__("Check out '%s' on %s website", 'charity-ngo'), $social_title, $website_name), ENT_QUOTES, 'UTF-8')) . "&url={$page_link}\">" . esc_html__('Twitter', 'charity-ngo') . "</a>
			<a href=\"https://pinterest.com/pin/create/button/?url={$page_link}&media={$pinterest_img}&description={$social_title}\">" . esc_html__('Pinterest', 'charity-ngo') . "</a>
		</div>
	</aside>
";
}
...
function charity_ngo_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'charity-ngo') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function charity_ngo_related($tag = 'h3', $more_posts = '', $no_posts = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
						...
						while ($r_query->have_posts()) : $r_query->the_post();
							echo "<div class=\"cmsmasters_owl_slider_item cmsmasters_single_slider_item\">
								<div class=\"cmsmasters_single_slider_item_outer\">";
									...
									charity_ngo_thumb(get_the_ID(), 'cmsmasters-blog-masonry-thumb', true, false, true, false, true, true, false, false, false, 'cmsmasters_theme_icon_image');
...
function charity_ngo_author_avatar($template_type = 'page') {
...
/* Get Category List */
function charity_ngo_get_the_category_list($cmsmasters_id, $taxonomy, $sep = '', $before = '', $after = '') {
...
if (!function_exists('charity_ngo_get_post_pings')) {

function charity_ngo_get_post_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (pings_open($id) && sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'charity-ngo') . '</' . esc_html($tag) . '>' . "\n" .
