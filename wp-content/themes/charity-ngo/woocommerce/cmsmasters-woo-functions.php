...
function charity_ngo_woocommerce_cart_dropdown() {
	global $woocommerce;
...
function charity_ngo_woocommerce_add_to_cart_button() {
	global $woocommerce, 
		$product;
	...
	if ( 
		...
		echo '<div class="button_to_cart_wrap">' . 
			'<a href="' . esc_url($product->add_to_cart_url()) . '" data-product_id="' . esc_attr($product->id) . '" data-product_sku="' . esc_attr($product->get_sku()) . '" class="button_to_cart add_to_cart_button cmsmasters_add_to_cart_button ajax_add_to_cart product_type_simple" title="' . esc_attr__('Add to Cart', 'charity-ngo') . '">' . 
				'<span>' . esc_html__('Add to Cart', 'charity-ngo') . '</span>' . 
			'</a>' . 
			'<a href="' . esc_url($woocommerce->cart->get_cart_url()) . '" class="button_to_cart added_to_cart wc-forward" title="' . esc_attr__('View Cart', 'charity-ngo') . '">' . 
				'<span>' . esc_html__('View Cart', 'charity-ngo') . '</span>' . 
			'</a>' . 
		'</div>';
	}
}
...
/* Woocommerce Rating */
function charity_ngo_woocommerce_rating($icon_trans = '', $icon_color = '', $in_review = false, $comment_id = '', $show = true) {
	global $product;
	...
	$out = "
<div class=\"cmsmasters_star_rating\" itemscope itemtype=\"http://schema.org/{$itemtype}\" title=\"" . sprintf(esc_html__('Rated %s out of 5', 'charity-ngo'), $rating) . "\">
...
<span class=\"rating dn\"><strong itemprop=\"ratingValue\">" . esc_html($rating) . "</strong> " . esc_html__('out of 5', 'charity-ngo') . "</span>
</div>
";
...
function charity_ngo_woocommerce_price_format($format, $currency_pos) {
...
add_action('woocommerce_price_format', 'charity_ngo_woocommerce_price_format', 1, 2);
...
add_filter( 'woocommerce_sale_flash', 'charity_ngo_change_on_sale' );

function charity_ngo_change_on_sale() {
